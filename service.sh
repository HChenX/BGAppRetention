...
setZram() {
    ...
    { [[ $zramAll -gt 15000000 ]] && zramSize=20; } || {
        [[ $zramAll -gt 11000000 ]] && zramSize=16
    } || {
        [[ $zramAll -gt 7000000 ]] && zramSize=12
    } || {
        [[ $zramAll -gt 5000000 ]] && zramSize=10
    } || {
        [[ $zramAll -gt 3000000 ]] && zramSize=8
    } || { zramSize=6; }
    zramSize=16
...
setVm() {
    ...
    { [[ $zramAll -gt 15000000 ]] && waterM=350; } || {
        [[ $zramAll -gt 11000000 ]] && waterM=300
    } || {
        [[ $zramAll -gt 7000000 ]] && waterM=250
    } || {
        [[ $zramAll -gt 5000000 ]] && waterM=200
    } || {
        [[ $zramAll -gt 3000000 ]] && waterM=150
    } || { waterM=100; }
...
echo "手机品牌:$(getprop ro.product.brand)" >"$HChen"/log.txt
{
    ...
    echo "系统开机时间:$time"
    { [[ $app_retention == "yes" ]] && {
        echo "AppRetention已经安装:$(dumpsys package Com.HChen.Hook | grep versionName | cut -f2 -d '=')"
    }; } || {
